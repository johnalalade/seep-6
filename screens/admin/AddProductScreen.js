...
const AddProductScreen = ({ navigation, route }) => {
  ...
  const upload = async () => {
    ...
    formdata.append("file", {
      uri: image[0].uri,
      name: `${image[0].fileName}_XX4_`,
      type: `${image[0].type}/${image[0].uri.slice(
    ...
    fetch(
      "https://silicio.herokuapp.com/photos/upload",
      ImageRequestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  const addProductHandle = async () => {
    ...
    if (title == "") {
      ...
      formdata.append("file", {
        uri: files[0].uri,
        name: `${files[0].fileName}_xxx_${uuid.v4()}`,
        type: `${files[0].type}/${files[0].uri.slice(
          files[0].uri.lastIndexOf(".") - 1
        )}`,
      ...
      axios
        .post(network.serverip + "/product", formdata, {
          headers: {
